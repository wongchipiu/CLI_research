...
def test_tws_broker_submits_only_monotonic_close_orders(monkeypatch):
    ...
    broker._app.error(-1, 1_788_282_000, 1102, "data maintained", "")
    assert broker.is_connected() is True
    broker._app.error(-1, 1_788_282_001, 1101, "data lost", "")
    assert broker.is_connected() is False
...
def test_tws_broker_global_cancel_uses_order_cancel_request(monkeypatch):
    installed = _install_fake_ibapi(monkeypatch)
    broker = TwsBroker(BrokerConfig(expected_account="DU_TEST", dry_run=False))
    broker.connect()
    ...
    assert installed["client"].global_cancel_count == 1
    assert installed["client"].global_cancel_request.__class__.__name__ == "OrderCancel"
    broker.disconnect()
...
def _install_fake_ibapi(monkeypatch):
    ...
    order_cancel_module = types.ModuleType("ibapi.order_cancel")
    installed = {}
    ...
    class EClient:
        ...
        def reqGlobalCancel(self, order_cancel):
            self.global_cancel_count += 1
            self.global_cancel_request = order_cancel
    ...
    for name, module in {
        ...
        "ibapi.order_cancel": order_cancel_module,
    }.items():
        monkeypatch.setitem(sys.modules, name, module)
    order_cancel_module.OrderCancel = OrderCancel
    return installed
