...
- 默认 next_open_v1：decision[t] 在 t+1 开盘执行，缺失开盘报价不能用收盘价代替；legacy_same_close 仅显式历史比较。
- S2 只用训练段选参，最终测试使用情况写入 --study-file；实用指南见 docs/USER_GUIDE.md。
- S3 模拟信号不携带执行价格；gpt_quant 只在后续常规交易时段使用新鲜价格处理一次。
