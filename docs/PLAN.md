...
- 后续 S2（需单独确认）：Paper 连续运行验收、通知渠道接入与实盘上线评审。
...
- **S1 已完成：** 修复冻结持仓后的资金约束，并分离估值价格和可交易状态；设计及验证见 [M7_S1.md](M7_S1.md)。
- S4–S9 为后续建议条目，范围与验收见整合计划第 6 节；每个对话仍只做一个 S 条目。
- **S2 已完成（2026-08-29）：** 已实现 60/20/20 顺序划分、仅训练选参、逐折训练选参、一次性最终测试、次日开盘成交、新旧证据隔离和实用指南；设计与验收见 [M7_S2.md](M7_S2.md)。
- **S3 已完成（2026-08-29）：** 已实现模拟账本单调日期、固定日初权益、信号唯一键与重放保护、新鲜执行价格、证据变更熔断和重启恢复；指南与验收见 [M7_S3.md](M7_S3.md)。
- 下一项为 M7-S4：统一入口、工作目录和版本化数据/信号契约；不得同时扩展 Radar 或连接实盘券商。
