...
当前边界：已冻结 XNYS 与 `XSHG+XSHE` 交易日历，但未实现所有 A 股板块特殊交易规则、真实订单撮合或分钟级成交；免费来源的时效和复权一致性仍需逐次验证。日历版本覆盖范围外会直接拒绝并要求升级。自定义策略必须通过“改变未来数据不改变历史信号”的因果性测试；系统隔离了选参区间，但不会自动证明任意策略代码没有前视逻辑。
...
当前日历契约为 `exchange_calendars:4.13.2:XNYS` 和 `exchange_calendars:4.13.2:XSHG+XSHE`，能识别冻结版本覆盖范围内的节假日、XNYS 提前收市和 A 股午休。SZSE 与 SSE 在本系统中显式共用中国内地交易日安排；若未来出现不一致，必须升级为独立 XSHE provider，不能静默沿用。版本或覆盖范围不匹配时应升级代码与契约，不要手工伪造价格或日期。
